...
if(isset($_GET['startdate'])){
	...
	$q=sqlsrv_query($conn,"select u.id as idd,(u.equipment + ' ' + u.brand + ' ' + u.model + ' ' + u.type + ' ' + u.plateNo) as name,sum(f.mins) as tmin from downtimeflatdata f right join unit u on u.id=f.unitId 
		where f.date>='".$_GET['startdate']."' and f.date<='".$_GET['enddate']."' ".$vc."
		GROUP BY u.id,(u.equipment + ' ' + u.brand + ' ' + u.model + ' ' + u.type + ' ' + u.plateNo)
	...
	$q=sqlsrv_query($conn,"select (u.equipment + ' ' + u.brand + ' ' + u.model + ' ' + u.type + ' ' + u.plateNo) as name, u.* from unit u where u.id not in (".$wdata.")");
	while($r=sqlsrv_fetch_array($q)){		
