...
$lq=sqlsrv_query($conn,"select CONVERT(VARCHAR(19),date_needed) as needed,CONVERT(VARCHAR(19),addedAt) as added,CONVERT(VARCHAR(19),lastStatusChanged) as lastchanged,* from vehicle_request where dept='".$_SESSION['esdvms_requestor_edept']."' order by id desc");
while($l=sqlsrv_fetch_array($lq)){    
    ...
    $spreadsheet->getActiveSheet()
        ...
        ->setCellValueByColumnAndRow(2, $row, date('m/d/Y h:i A',strtotime($l['needed'])))
        ->setCellValueByColumnAndRow(3, $row, date('m/d/Y h:i A',strtotime($l['added'])))
        ->setCellValueByColumnAndRow(4, $row, $l['purpose'])
        ->setCellValueByColumnAndRow(5, $row, $l['status'])
        ->setCellValueByColumnAndRow(6, $row, date('m/d/Y h:i A',strtotime($l['lastchanged'])))
