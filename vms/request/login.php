...
if(isset($_GET['act'])){
	if($_GET['act']=='login'){
		 $check_if_dept_user=sqlsrv_fetch_array(sqlsrv_query($conn,"SELECT * FROM users WHERE domain='".$_POST['loginname']."' and active=1 and isdepartment=1"));
		 if($check_if_dept_user['id']){
		 	LoginSuccessful($userlogin); // Add Logs
			session_start();
			$_SESSION['esdvms_requestor_id']=$check_if_dept_user['id'];
			$_SESSION['esdvms_requestor_username']=$_POST['loginname'];
			$_SESSION['esdvms_requestor_ename']=$check_if_dept_user['fullname'];
			$_SESSION['esdvms_requestor_edept']=$check_if_dept_user['dept'];
			$_SESSION['esdvms_requestor_erole']=$check_if_dept_user['role'];			
...
<body class="login">
	...
	<?php } ?>
<div class="logo">
	<img src="../metronic/ecs.png" alt="" width="400">
</div>
	...
	<form class="login-form" action="login.php?act=login" method="post">
		...
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Domain Account" name="loginname" id="loginname"/>
			</div>
		</div>
		<div class="form-group">
