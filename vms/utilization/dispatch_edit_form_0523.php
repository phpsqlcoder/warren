...
$history = sqlsrv_fetch_array(sqlsrv_query($conn,"select top 1 odometer_end from dispatch where unitId='".$r['unitId']."' and id<>'".$r['id']."' order by odometer_end desc"));
}
            ...
            while($unit = sqlsrv_fetch_array($units)){
               ...
               if($check_if_down['id']){
                  ...
                  if($check_if_no_booking['id']){
                     ...
                     $hhh = sqlsrv_fetch_array(sqlsrv_query($conn,"select top 1 odometer_end from dispatch where unitId='".$unit['id']."' and id<>'".$r['id']."' order by odometer_end desc"));                                               
...
<form action="" method="post" id="ddform">
   ...
   <div style="text-align: right;" class="col-md-12">
      ...
      <a class="btn yellow" href="#" onclick="$('#is_print').val(1); $('#ddform').submit();">
...
<?php 
   ...
   <form method="post" action="" id="form_closing">
      ...
      <div class="form-group col-md-12">
         <div class="col-md-4">
            <label class="control-label">Return Date<i class="font-red"> *</i></label>
            <div class="input-group form_date date col-md-12" data-date="yyyy-mm-dd" data-date-format="yyyy-mm-dd" data-link-field="date_return">
               <div class="input-icon">
                  <i class="fa fa-calendar font-yellow"></i>
         ...
         <div class="col-md-4">
            ...
            <?php 
               if ($r['isPrinted'] == 1) {
               ?>
                  <input readonly type="text" name="fuel_type" class="form-control" value="<?= $r['fuel_added_type']; ?>">
      ...
      <div class="form-group col-md-12">
         <div class="col-md-4">
            ...
            <div class="input-icon">
               <i class="icon-calculator font-yellow"></i>
               <input <?= $isPrinted; ?> type="text" class="form-control" id="uom" name="uom" value="Liters">
            </div>
         </div>
...
<script>
   ...
   window.onload = function () { 
      ...
      $('#form_closing').on('submit', function() {
        // alert(parseFloat($('#odom_startn').val())+ ' fff  '+ parseFloat($('#last_odo').val()));
         if( parseFloat($('#odom_startn').val()) < parseFloat($('#last_odo').val())  ){
            alert('Odometer start must be greater than previous odometer which is: '+$('#last_odo').val());
            return false;   
         }
         
         if( parseFloat($('#odom_e').val()) <= parseFloat($('#odom_startn').val())  ){
