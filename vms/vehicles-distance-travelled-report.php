                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        if(isset($_GET['start'])) {
                            ...
                            $strQuery = sqlsrv_query($conn,"SELECT TOP(10) type,odometer_start,odometer_end,odometer_end - odometer_start AS sub FROM dispatch WHERE addedDate BETWEEN '$from' AND '$to' ORDER BY sub DESC");

                            if ($strQuery) {

                                $arrData = array(
                                    "chart" => array(
                                        "caption" => "DISTANCE TRAVELLED PER VEHICLE",
                                        "xAxisName" => "",
                                        "showValues" => "1",
                                        "theme" => "fusion"
                                    )
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                                            ...
                                            <tbody>    
                                                ...
                                                if(isset($_GET['start'])){
                                                    ...
                                                    $query = sqlsrv_query($conn,"SELECT top(10) type,odometer_start,odometer_end,odometer_end - odometer_start AS sub FROM dispatch WHERE addedDate BETWEEN '$from' AND '$to' ORDER BY sub DESC");

                                                    while ($u= sqlsrv_fetch_array($query)) {
                                                        echo '<tr>
                                                        <td>'.strtoupper($u['type']).'</td>
                                                        <td>'.intval($u['odometer_start']).' KM'.'</td>
                                                        <td>'.intval($u['odometer_end']).' KM'.'</td>
                                                        <td>'.intval($u['sub']).' KM'.'</td>
                                                        </tr>';
                                                    }
