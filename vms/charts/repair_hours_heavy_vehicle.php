...
$iq=sqlsrv_query($conn,"select top 10 u.required_availability_hours,u.name,sum(trepair_hours) as total from downtime d left join unit u on u.id=d.unitId
  where d.tdowntime>0 and d.active=1 and u.type='Heavy Equipment' ".$cond." group by u.required_availability_hours,u.name ORDER by sum(trepair_hours) desc");
$idata='';
