...
if(isset($_GET['startdate'])){
	...
	$q=sqlsrv_query($conn,"select u.id as idd,CONCAT(equipment,brand,model,avNo,location) as name,sum(f.mins) as tmin from downtimeflatdata f right join unit u on u.id=f.unitId 
		where f.date>='".$_GET['startdate']."' and f.date<='".$_GET['enddate']."' ".$vc."
		GROUP BY u.id,equipment,brand,model,avNo,location
