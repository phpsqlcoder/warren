...
if(strlen($_GET['s_equipment'])>1){
	$cond.=" and u.location='".$_GET['s_equipment']."'";
}
				...
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>			
							...
							<select class="form-control input-sm" name="s_equipment">
				 				<?php
				 						echo '<option value="" selected="selected"> - Select Equipment -';
						...
						<li>							
							<select class="form-control input-sm" name="s_type">
				 				<?php
				 						echo '<option value="" selected="selected"> - Select Type -';
						...
						<li>
							<select class="form-control input-sm" name="s_name">
				 				<?php
				 					...
				 					$uq=sqlsrv_query($conn,"select distinct  (equipment + ' ' + brand + ' ' + model + ' ' + type + ' ' + plateNo) as name from unit order by (equipment + ' ' + brand + ' ' + model + ' ' + type + ' ' + plateNo)");
				 					while($u=sqlsrv_fetch_array($uq)){
				 						$select='';
				 						if(isset($_GET['s_name'])){
				 							if($u['name']==$_GET['s_name'] && $u['name']!=''){
				 								$select=' selected="selected"';
				 							}
				 						}
			...
			<div class="row ">
				<div class="col-md-12 col-sm-12">
									...
									<?php 
										...
										$seq=$lt['totald'];
										$seq=0;
										$lq=sqlsrv_query($conn,"select CONVERT(VARCHAR(19),d.dateStart) as ds,CONVERT(VARCHAR(19),d.dateEnd) as de,d.*, (u.equipment + ' ' + u.brand + ' ' + u.model + ' ' + u.type + ' ' + u.plateNo) as uni from downtime d 
											left join unit u on u.id=d.unitId where d.id>0 and active=1 ".$cond." order by d.id desc");
